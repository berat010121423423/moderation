...
//-------------------------------------------------------------------------------\\
  if(!["805738640894328843"].some(role => message.member.roles.cache.get(role)) && (!message.member.hasPermission("VIEW_AUDIT_LOG"))) 
...
exports.conf = {
  enabled: true,
  guildOnly: true,
  aliases: ["sicil-sıfırla",'sicilsıfır','sicil sıfırla'],
  PermLevel: 0
};
