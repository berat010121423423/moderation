...
if(!["805738640901668888"].some(role => message.member.roles.cache.get(role)) && (!message.member.hasPermission("VIEW_AUDIT_LOG"))) 
...
const mutelog = message.guild.channels.cache.find(c => c.id === '805738644710359064')//Mute logu
const muterol = message.guild.roles.cache.find(r => r.id === '805738640864837642')//Muteli rolü

//-------------------------------------------------------------------------------\\

let member = message.guild.member(message.mentions.users.first() || message.guild.members.cache.get(args[0]));
if (!member) return message.channel.send(new MessageEmbed().setColor('0x800d0d').setDescription(`${message.author}, Mute Yiceeek Kullanıcıyı Belirt. !`))
...
.replace("sa", "h")
.replace("gün", "d");
if (!zaman) { message.channel.send(new MessageEmbed().setColor('0x800d0d').setDescription(`Lütfen Muteleyeceğim Süreyide Belirt.`));
} else {
let sebep = args[2]
if(!sebep) return message.channel.send(new MessageEmbed().setColor('0x800d0d').setDescription(`Lütfen Neden Mute Atıcağımıda Yazarmısın.`))  
