...
if(!["805738640901668890"].some(role => message.member.roles.cache.get(role)) && (!message.member.hasPermission("VIEW_AUDIT_LOG"))) 
return message.channel.send(new MessageEmbed().setDescription(`<a:croz:806650760104247327> **Bir Hata Oluştu** <a:croz:806650760104247327>\n\`Komutu kullanmak için yetkin bulunmamakta.\``).setColor('0x800d0d').setAuthor(message.member.displayName, message.author.avatarURL({ dynamic: true })).setTimestamp()).then(x => x.delete({timeout: 5000}));
